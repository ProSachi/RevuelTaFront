...
export function ProveedorAutenticacion({ children }) {
  ...
  // Cargar sesión persistida al iniciar o recargar la aplicación
  useEffect(() => {
    try {
   ...
   */
  const iniciarSesion = async ({ correo, contrasena, _recordarContrasena = false }) => {
    try {
   ...
   */
  const registrarUsuario = async ({ correo, _contrasena, recibirNovedades = false }) => {
    try {
      await new Promise((resolve) => setTimeout(resolve, 200))
      const nuevoUsuario = {
        id: Date.now(),
